...
from .configuration_clip import CLIPConfig, CLIPTextConfig, MYCLIPVisionConfig
...
@dataclass
class MYCLIPVisionModelOutput(ModelOutput):
    """
...
class CLIPOutput(ModelOutput):
    ...
    Args:
        ...
        image_embeds(`torch.FloatTensor` of shape `(batch_size, output_dim`):
            The image embeddings obtained by applying the projection layer to the pooled output of [`EvaMYCLIPVisionModel`].
        text_model_output(`BaseModelOutputWithPooling`):
            The output of the [`EvaCLIPTextModel`].
        vision_model_output(`BaseModelOutputWithPooling`):
            The output of the [`EvaMYCLIPVisionModel`].
    """
...
class CLIPVisionEmbeddings(nn.Module):
    def __init__(self, config: MYCLIPVisionConfig):
        super().__init__()
        self.config = config
...
class CLIPAttention(nn.Module):
    ...
    def __init__(self, config, rope=None):
        ...
        self.inner_self_attn_ln = norm_layer(self.embed_dim)

        self.attn_drop = nn.Dropout(config.attn_drop)
        # self.inner_attn_ln = norm_layer(self.embed_dim)
        # self.proj = nn.Linear(self.embed_dim, self.embed_dim)
        self.proj_drop = nn.Dropout(config.proj_drop)
        self.xattn = config.xattn
        self.xattn_drop = config.attn_drop
    ...
    def forward(
        ...
        # if self.rope:
        #     # slightly fast impl
        #     query_states_t = query_states[:, :, 1:, :]
        #     ro_query_states_t = self.rope(query_states_t)
        #     query_states = torch.cat((query_states[:, :, :1, :], ro_query_states_t), -2).type_as(value_states)

        #     key_states_t = key_states[:, :, 1:, :]
        #     ro_key_states_t = self.rope(key_states_t)
        #     key_states = torch.cat((key_states[:, :, :1, :], ro_key_states_t), -2).type_as(value_states)
        ...
        # attn_output = self.inner_self_attn_ln(attn_output)
...
class SwiGLU(nn.Module):
    ...
    def __init__(self, config):
        ...
        # norm_layer = partial(FusedLayerNorm, eps=1e-6) if config.fusedLN else partial(LayerNorm, eps=1e-6)
        norm_layer = partial(LayerNorm, eps=1e-6)
        self.ffn_ln = norm_layer(config.intermediate_size)
...
class CLIPEncoderLayer(nn.Module):
    def __init__(self, config: CLIPConfig, rope=None):
        super().__init__()
        self.config = config
        self.embed_dim = config.hidden_size
        self.post_layernorm = config.post_layernorm if config.post_layernorm is not None else False
        self.self_attn = CLIPAttention(config, rope=rope) 
        # norm_layer = partial(FusedLayerNorm, eps=1e-6) if config.fusedLN else partial(LayerNorm, eps=1e-6)
        norm_layer = partial(LayerNorm, eps=1e-6)
        self.layer_norm1 = norm_layer(self.embed_dim)
        ...
        self.layer_norm2 = norm_layer(self.embed_dim)
    def forward(
        self,
        hidden_states: torch.Tensor,
...
class MYCLIPPreTrainedModel(PreTrainedModel):
    """
    ...
    def _init_weights(self, module):
        ...
        #     )
        # elif isinstance(module, MYCLIPVisionModelWithProjection):
        #     nn.init.normal_(
        #         module.visual_projection.weight,
...
class CLIPEncoder(nn.Module):
    ...
    def __init__(self, config: CLIPConfig, rope=None):
        super().__init__()
        self.config = config
        self.layers = nn.ModuleList([CLIPEncoderLayer(config, rope=rope) for _ in range(config.num_hidden_layers)])
...
class CLIPTextModel(MYCLIPPreTrainedModel):
...
class Transformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, attn_mask: Optional[torch.Tensor] = None, output_hidden_states: Optional[bool] = None):
        encoder_states = () if output_hidden_states else None
        hidden_states = x
        for r in self.resblocks:
            ...
            if self.grad_checkpointing and not torch.jit.is_scripting():
                hidden_states = checkpoint(r, hidden_states, attn_mask)
            else:
                hidden_states = r(hidden_states, attn_mask=attn_mask)
        if output_hidden_states:
            encoder_states = encoder_states + (hidden_states.permute(1, 0, 2),)

        return BaseModelOutput(
            last_hidden_state=hidden_states.permute(1, 0, 2), hidden_states=encoder_states, attentions=None
        )
...
class CLIPVisionTransformer(nn.Module):
    def __init__(self, config: MYCLIPVisionConfig):
        ...
        # else: 
        #     self.rope = None
        # self.encoder = CLIPEncoder(config, rope=self.rope)
        # # self.post_layernorm = nn.LayerNorm(embed_dim, eps=config.layer_norm_eps)
        # # norm_layer = partial(FusedLayerNorm, eps=1e-6) if config.fusedLN else partial(LayerNorm, eps=1e-6)
        # norm_layer = partial(LayerNorm, eps=1e-6)
        # self.post_layernorm = norm_layer(embed_dim)
        self.config = config
        ...
        global_average_pool = config.global_average_pool
    ...
    @replace_return_docstrings(output_type=BaseModelOutputWithPooling, config_class=MYCLIPVisionConfig)
    def forward(
        self,
        ...
        x = self.conv1(pixel_values.to(dtype=target_dtype))  # shape = [*, width, grid, grid]
        ...
        encoder_outputs = self.transformer(x, output_hidden_states=output_hidden_states)
        ...
        pooled_output = self.ln_post(pooled_output)
...
class MYCLIPVisionModel(MYCLIPPreTrainedModel):
    config_class = MYCLIPVisionConfig
    ...
    def forward(
        ...
        >>> import requests
        >>> from transformers import AutoProcessor, MYCLIPVisionModel
        >>> model = MYCLIPVisionModel.from_pretrained("openai/clip-vit-base-patch32")
...
class CLIPModel(MYCLIPPreTrainedModel):
    config_class = CLIPConfig

    def __init__(self, config: CLIPConfig):
        super().__init__(config)

        if not (type(config.text_config).__name__ == "CLIPTextConfig"):
            raise ValueError(
                "config.text_config is expected to be of type CLIPTextConfig but is of type"
                f" {type(config.text_config)}."
            )

        if not (type(config.vision_config).__name__ == "MYCLIPVisionConfig"):
            raise ValueError(
                "config.vision_config is expected to be of type MYCLIPVisionConfig but is of type"
    ...
    def get_image_features(
        ...
        Returns:
            ...
            applying the projection layer to the pooled output of [`EvaMYCLIPVisionModel`].
        Examples:
        ```python
        >>> from PIL import Image
        >>> import requests
...
class CLIPTextModelWithProjection(MYCLIPPreTrainedModel):
    config_class = CLIPTextConfig
...
class MYCLIPVisionModelWithProjection(MYCLIPPreTrainedModel):
    config_class = MYCLIPVisionConfig
    main_input_name = "pixel_values"
    ...
    @add_start_docstrings_to_model_forward(EvaCLIP_VISION_INPUTS_DOCSTRING)
    @replace_return_docstrings(output_type=MYCLIPVisionModelOutput, config_class=MYCLIPVisionConfig)
    def forward(
        self,
        pixel_values: Optional[torch.FloatTensor] = None,
        output_attentions: Optional[bool] = None,
        output_hidden_states: Optional[bool] = None,
        return_dict: Optional[bool] = None,
    ) -> Union[Tuple, MYCLIPVisionModelOutput]:
        r"""
        Returns:
        Examples:
        ```python
        >>> from PIL import Image
        >>> import requests
        >>> from transformers import AutoProcessor, MYCLIPVisionModelWithProjection
        >>> model = MYCLIPVisionModelWithProjection.from_pretrained("openai/clip-vit-base-patch32")
        >>> processor = AutoProcessor.from_pretrained("openai/clip-vit-base-patch32")
        >>> url = "http://images.cocodataset.org/val2017/000000039769.jpg"
        >>> image = Image.open(requests.get(url, stream=True).raw)
        >>> inputs = processor(images=image, return_tensors="pt")
        ...
        return MYCLIPVisionModelOutput(
            image_embeds=image_embeds,
