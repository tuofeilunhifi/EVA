...
#         if config.attn_head_dim is not None:
...
#         if self.subln:
#             self.q_proj = nn.Linear(config.dim, all_head_dim, bias=False)
#             self.k_proj = nn.Linear(config.dim, all_head_dim, bias=False)
#             self.v_proj = nn.Linear(config.dim, all_head_dim, bias=False)
#         else:
#             self.qkv = nn.Linear(config.dim, all_head_dim * 3, bias=False)
...
#         self.inner_attn_ln = norm_layer(all_head_dim) if config.subln else nn.Identity()
#         # self.proj = nn.Linear(all_head_dim, all_head_dim)
#         self.proj = nn.Linear(all_head_dim, config.dim)
...
class EvaCLIPEncoderLayer(nn.Module):
    def __init__(self, config: EvaCLIPConfig, rope=None):
        ...
        self.self_attn = EvaCLIPAttention(config, rope=rope) 
        # norm_layer = partial(FusedLayerNorm, eps=1e-6) if config.fusedLN else partial(LayerNorm, eps=1e-6)
        norm_layer = partial(LayerNorm, eps=1e-6)
...
class EvaCLIPEncoder(nn.Module):
    ...
    def __init__(self, config: EvaCLIPConfig, rope=None):
        super().__init__()
        self.config = config
        self.layers = nn.ModuleList([EvaCLIPEncoderLayer(config, rope=rope) for _ in range(config.num_hidden_layers)])
...
class EvaCLIPVisionTransformer(nn.Module):
    def __init__(self, config: EvaCLIPVisionConfig):
        ...
        if config.rope:
            ...
            self.rope = None
        self.encoder = EvaCLIPEncoder(config, rope=self.rope)
...
@add_start_docstrings(
    """The vision model(with dino) from EvaCLIP without any head or projection on top.""",
    EvaCLIP_START_DOCSTRING,
)
class EvaCLIPWithDINOVisionModel(EvaCLIPPreTrainedModel):
    config_class = EvaCLIPVisionConfig
    main_input_name = "pixel_values"
    ...
    def __init__(self, config: EvaCLIPVisionConfig):
        ...
        # Initialize weights and apply final processing
        # self.post_init()
        self.dino_model = Dinov2Model.from_pretrained(config.dino_model_name)
    ...
    def forward(
        ...
        visual_outputs = self.vision_model(
            pixel_values=pixel_values,
            output_attentions=output_attentions,
            output_hidden_states=output_hidden_states,
            return_dict=return_dict,
        )
        dino_visual_outputs = self.dino_model(pixel_values=pixel_values, output_hidden_states=True)
        ...
        #     return_dict=return_dict,
        # )
        last_hidden_state = torch.cat([visual_outputs.last_hidden_state, dino_visual_outputs.last_hidden_state], dim=-1)
        hidden_states = [torch.cat([visual_outputs.hidden_states[:-5][i], dino_visual_outputs.hidden_states[:-5][i]], dim=-1) for i in range(5)]
        pooler_output = torch.cat([visual_outputs.pooler_output, dino_visual_outputs.pooler_output], dim=-1)
        return BaseModelOutputWithPooling(
            last_hidden_state=last_hidden_state,
